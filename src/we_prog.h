...
/********** prototypes ****************/
struct dirfile** e_make_prj_opt(we_window_t* f);
int e_rel_brkwtch(we_window_t* f);
int e_prj_ob_file(we_window_t* f);
int e_make_error_list(we_window_t* f);
int e_d_car_ret(we_window_t* f);
int e_prog_switch(we_window_t* f, int c);

int print_to_end_of_buffer(BUFFER* b, char* str, int wrap_limit);

/*   we_prog.c   */

int e_compile(we_window_t* f);
int e_p_make(we_window_t* f);
int e_run(we_window_t* f);
int e_c_project(we_window_t* f);
int e_free_arg(char** arg, int argc);
int e_comp(we_window_t* f);
int e_exec_inf(we_window_t* f, char** argv, int n);
int e_print_arg(FILE* fp, char* s, char** argv, int n);
int e_show_error(int n, we_window_t* f);
int e_previous_error(we_window_t* f);
int e_next_error(we_window_t* f);
...
int e_d_car_mouse(we_window_t* f);
int e_add_arg(char*** arg, char* str, int n, int argc);
int e_new_message(we_window_t* f);
int e_p_cmp_mess(char* srch, BUFFER* b, int* ii, int* kk, int ret);
int e_project_name(we_window_t* f);
int e_wrt_prj_fl(we_window_t* f);
int e_p_update_prj_fl(we_window_t* f);
int freedfN(struct dirfile** df, int n);
int e_p_red_buffer(BUFFER* b);
int e_read_var(we_window_t* f);
