...
int e_sh_def(we_window_t* f);
int e_sh_nxt_def(we_window_t* f);
int e_nxt_brk(we_window_t* f);
int e_p_beautify(we_window_t* f);

/*   we_progn.c  */

int e_scfbol(int n, int mcsw, unsigned char* str, struct wpeSyntaxRule* cs);
int e_sc_all(we_window_t* f, int sw);
int e_program_opt(we_window_t* f);
void e_pr_c_line(int y, we_window_t* f);
...
int e_show_nm_f(char* name, we_window_t* f, int oldn, char** oldname);
